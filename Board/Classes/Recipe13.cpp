...
CCScene* Recipe13Scene::scene()
{
    // 'scene' is an autorelease object
    CCScene *scene = CCScene::create();
    
    // 'layer' is an autorelease object
	Recipe13Scene *layer = Recipe13Scene::create();
...
bool Recipe13Scene::init()
{
	...
	this->schedule(schedule_selector(Recipe13Scene::gameLogic), 3.0);
...
void Recipe13Scene::addFood()
{
	...
	CCCallFuncN* actionMoveDone =
		...
		callfuncN_selector(Recipe13Scene::spriteMoveFinished));
	//------------------
	food->runAction(
...
void Recipe13Scene::gameLogic(float delta)
{
	this->addFood();
}

void Recipe13Scene::spriteMoveFinished(CCNode* sender)
{
...
void Recipe13Scene::ccTouchesBegan(CCSet* touches, CCEvent* event)
{
...
void Recipe13Scene::update(float dt)
{
	//	うっほい君を取得
	CCSprite* player = (CCSprite*)this->getChildByTag(1);
	CCRect playerRect =
	...
	if (playerRect.intersectsRect(foodRect))
	{
		...
		///	01秒後にeatを１度だけ実行させる
		this->scheduleOnce(schedule_selector(Recipe13Scene::eat), 0.1f);
	}
}

void Recipe13Scene::eat(float dt)
{
	//	うっほい君を取得
	CCSprite* player = (CCSprite*)this->getChildByTag(1);
	///	うっほい君のイメージ変更
